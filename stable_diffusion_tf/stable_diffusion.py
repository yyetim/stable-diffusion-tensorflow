...
def get_model(img_height, img_width, download_weights=True):
    ...
    diffusion_input_specs = [
        tf.TensorSpec(
            shape=(None,) + input_shapes[0],
            dtype=tf.float32,
            name=diffusion_input_names[0],
        ),
        tf.TensorSpec(
            shape=(None,) + input_shapes[1],
            dtype=tf.double,
            name=diffusion_input_names[1],
        ),
        tf.TensorSpec(
            shape=(None,) + input_shapes[2],
            dtype=tf.double,
            name=diffusion_input_names[2],
        ),
    ]
    ...
    output_model_dir = None  # "/tmp/tmpzd02msqr"

    diffusion_model = tf.keras.models.Model(diffusion_inputs, unet(diffusion_inputs))
    ...
    if download_weights:
        decoder.load_weights(
            tf.keras.utils.get_file(
                ...
                file_hash="76925d9a8dd5235113f74667ad1a80149e941d65b1f80e8993e722dec2629a21",
            )
        )

        text_encoder.load_weights(
            tf.keras.utils.get_file(
                "text_encoder.h5",
                "https://huggingface.co/divamgupta/stable-diffusion-tensorflow/resolve/main/text_encoder.h5",
                hash_algorithm="sha256",
                file_hash="3e9645f40c00d5696aca18393c2c49ef825f495f2a312d14f1e537269329617f",
            )
        )

        if not output_model_dir:
            diffusion_model.load_weights(
                tf.keras.utils.get_file(
                    "diffusion_model.h5",
                    "https://huggingface.co/divamgupta/stable-diffusion-tensorflow/resolve/main/diffusion_model.h5",
                    hash_algorithm="sha256",
                    file_hash="c28f8a1f6296d8eb4d9469041065e480f823871c1dcc7689be9e8f4cc7e048a0",
                )
            )
...
def text2image(
    ...
    # now lets get the unconditional context
    ...
    tokens_unconditional = np.array(tokens_unconditional)[None].astype("int32")
    tokens_unconditional = np.repeat(tokens_unconditional, batch_size, axis=0)

    tokens_unconditional = tf.convert_to_tensor(tokens_unconditional)
    unconditional_context = text_encoder([tokens_unconditional, pos_ids])
    ...
    def get_model_output(latent, t):
        ...
        unconditional_latent = diffusion_model([latent, t_emb, unconditional_context])
        latent = diffusion_model([latent, t_emb, context])
    ...
    return np.clip(decoded.numpy(), 0, 255).astype("uint8")
